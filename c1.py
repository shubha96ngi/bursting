...
def run_syn(syn_model, title, run_duration=200., sp_times=(10, 20, 30), **kwargs):
    ...
    #print('spike=', neu2.spike)
    runner = bp.DSRunner(net, monitors=['pre.spike', 'post.spike', 'syn.g', 'post.input','syn.spike_arrival_time','pre.V','post.V'],inputs=['pre.input', 0])
    #runner = bp.DSRunner(net, monitors=['V', 'spike'], inputs=['input', 1.2])
    # print('spike_time=', runner.mon['pre.spike']) #['spike_arrival_time'])
    runner.run(run_duration)

    # print(runner.mon['pre.spike'].shape)
    # # how to change this value
    # #pre.spike.value[:5, 0] = [True, True, True, True, True]
    # print(type(pre.spike.value))
    ...
    plt.plot(runner.mon.ts, runner.mon['post.V'][:,0], label='V')
    plt.legend(loc='upper right')
    plt.xlabel(r'$t$ (ms)')
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
...
class AMPA(bp.synapses.TwoEndConn):
    ...
    def update(self):
        ...
        T = 1 / (1 + bm.exp(-(self.pre.spike - neu.V_th) / 2))
        ...
        # 电导模式下计算突触后电流大小
        self.post.input += self.g_max *self.g.sum(axis=0) * (self.E - self.post.V)

if __name__ == '__main__':
    #run_syn(AMPA, title='AMPA Synapse Model', sp_times=[25, 50, 75, 100, 160], g_max=0.1)
    ampa = AMPA(neu, neu, bp.conn.FixedProb(0.1))
    #print('time=', ampa.spike_arrival_time)
    run_syn(AMPA, title='AMPA Synapse Model', g_max=0.5)
